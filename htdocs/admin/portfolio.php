...
if ($_GET["preview"] > "") {
    ...
    $sqlPagePreview = mysqli_query($db_conn, "SELECT id, content FROM pages WHERE id=" . $pagePreviewId . " LIMIT 1;");
    ...
    <div class="col-lg-8">
        ...
        if ($_GET["newpage"] || $_GET["editpage"]) {
            ...
            if ($_GET["editpage"]) {
                ...
                $sqlPages = mysqli_query($db_conn, "SELECT id, title, thumbnail, content, active, datetime FROM pages WHERE id=" . $thePageId . " LIMIT 1;");
            ...
            <form role="pageForm" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <select class="form-control" name="page_status">
                        <option value="1" <?php if ($_GET["editpage"]) {
                            echo $selActive1;
                        } ?>>Active
                        </option>
                        <option value="0" <?php if ($_GET["editpage"]) {
                            echo $selActive0;
                        } ?>>Draft
                        </option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="page_title" value="<?php if ($_GET["editpage"]) {
                        echo $row['title'];
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control tinymce" rows="20" name="page_content"><?php if ($_GET["editpage"]) {
                            echo $row['content'];
                        } ?></textarea>
                </div>
                <div class="form-group">
                    <span><?php if ($_GET["editpage"]) {
                            echo "Updated: " . date('m-d-Y, H:i:s', strtotime($row['datetime']));
                        } ?></span>
                </div>
        ...
        $sqlSetup = mysqli_query($db_conn, "SELECT portfolioheading FROM setup LIMIT 1;");
        ...
        ?>
            ...
            <button type="button" class="btn btn-default" onclick="window.location='?newpage=true';"><i
                        class='fa fa-fw fa-paper-plane'></i> Create a New Page
            </button>
            <h2>Pages</h2>
            <div class="table-responsive">
                ...
                <form role="portfolioForm" method="post" action="">
                    <div class="form-group">
                        <label>Heading</label>
                        <input class="form-control" name="main_heading"
                               value="<?php echo $rowSetup['portfolioheading']; ?>" placeholder="My Portfolio">
                    </div>
